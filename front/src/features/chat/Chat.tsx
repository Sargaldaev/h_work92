...
import { addMessages, setCurrentMessageForUser, setMessages, setUsers, } from '../../store/chat/chatSlice.ts';
import { useAppDispatch, useAppSelector } from '../../app/hook.ts';
import { connect } from 'react-redux';

const Chat = () => {
  const dispatch = useAppDispatch();
  const { messages, onlineUsers, currentMessageForUser } = useAppSelector(
    (state) => state.chat,
  );
  ...
  const sendMessage = (message: MessageForm) => {
    ...
    const payload: MessageRequest = {
      user: user._id,
      text: message.text,
      forUser: currentMessageForUser ? currentMessageForUser._id : null,
    };
    ...
    dispatch(setCurrentMessageForUser(null));
  };
